...
def manual_eval(model):
    ...
    s_output = transformer.sample_output(model, i_seq[:10], EOS.index, PAD.index, t_seq.size(1))
    ...
    s_output = transformer.sample_output(model, i_seq, EOS.index, PAD.index, t_seq.size(1))
...
def main():
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if epoch % 10 == 0:
            manual_eval(model)
            torch.save(model.state_dict(), 'saved_model_small.pkl')
