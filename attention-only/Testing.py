...
def main():
    ...
    model.load_state_dict(torch.load('model_5357.pkl'))
    model.eval()
    ...
    sampled_output = transformer.sample_output(model, input_seq[:10], EOS.index, PAD.index, target_seq.size(1))
    ...
    input_seq = input_seq.cuda()
    target_seq = target_seq.cuda()
    print('Input size:', input_seq.shape)
